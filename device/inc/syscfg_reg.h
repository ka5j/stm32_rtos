 ...
 * here is accessible.
 *
 * Field coverage is scoped to what routing a GPIO pin to an EXTI line
 * needs (EXTICR1-4) plus the I/O compensation cell control/status
 * (CMPCR) - MEMRMP (boot memory remap) and PMC (peripheral mode config,
 * Ethernet PHY interface selection - not applicable to the F446, which has
 * no Ethernet MAC) are modeled as bare registers for correct struct
 * layout/offsets but have no bit-level macros, since nothing in this
 * project uses them.
 */
#ifndef SYSCFG_REG_H
#define SYSCFG_REG_H
 ...
 *
 * RESERVED0 preserves the fixed offset to CMPCR (see the top-of-file note
 * above) - it's a real gap in the memory map, not a struct-packing
 * artifact.
 */
 ...
 *
 * Each EXTICRx register packs four 4-bit fields (one per EXTI line, 4
 * lines per register - EXTICR[0] covers EXTI0-3, EXTICR[1] covers
 * EXTI4-7, and so on); a line's field position is (line % 4) * 4 bits
 * wide, indexing EXTICR[line / 4]. Each field selects which GPIO port
 * feeds that EXTI line - these values apply to any of the 16 fields
 * across all four registers, not just one.
 */
