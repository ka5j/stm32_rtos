...
**In progress:** `drivers/` has its error-status contract (`drivers/inc/driver_status.h`, the `DriverStatus_e` every driver function uses per CONTRIBUTING.md's error-handling contract), a complete GPIO driver (`drivers/inc/gpio.h`, `drivers/src/gpio.c`: `gpioInit`/`gpioDeinit`/`gpioSetAlternateFunction`/`gpioWritePin`/`gpioReadPin`/`gpioTogglePin`), and RCC peripheral clock gating (`drivers/inc/rcc.h`, `drivers/src/rcc.c`: `rccGpioClockEnable`/`rccGpioClockDisable`/`rccUsart2ClockEnable`/`rccUsart2ClockDisable`/`rccSyscfgClockEnable`/`rccSyscfgClockDisable`/`rccPwrClockEnable`/`rccPwrClockDisable`) - all with 100% line and branch coverage via `make coverage`. RCC's HSI/HSE-to-PLL SYSCLK bring-up is not yet implemented (it involves a blocking hardware-ready poll, not host-testable the way clock gating is - see `rcc.h`'s file-level comment), and UART, NVIC, and SysTick drivers are not yet implemented either; `0.2.0` is reserved for `drivers/`'s completion (see [docs/VERSIONING.md](docs/VERSIONING.md)).
