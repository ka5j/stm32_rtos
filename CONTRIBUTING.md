...
2. **Matching unit test** — every `.c` file touched under `drivers/`, `api/`, `bsp/`, or `rtos/` (except `app/src/main.c`, the entry point) requires a `tests/unit/test_<name>.c`. Every `*_reg.h` touched under `core/inc`/`device/inc` must be `#include`d by at least one file in `tests/unit/`. CI does not enforce a 1:1 naming rule for this, but the repository convention is one test file per register header (`test_<peripheral>_reg.c`, e.g. `gpio_reg.h` → `test_gpio_reg.c`). All test files are aggregated into a single `run_tests` binary by `tests/unit/test_runner.c`, which owns `main()`/`setUp()`/`tearDown()`; Unity permits only one definition of each per binary, so individual test files declare no `main()` of their own. Reaching each test therefore needs a hand-written `extern` declaration and a `RUN_TEST()` call in that file, and a test missing either one still compiles and still passes — it simply never runs. `make test` runs `tools/check_test_registration.awk` to catch that, since nothing else does: `make coverage` would notice for a driver test, but its filter is `drivers/src` only, so for the `test_<peripheral>_reg.c` files a forgotten registration is otherwise completely silent.

## `const` on a register-block parameter

A register-block parameter is `const` when the function's effect on that peripheral is nil — it observes and nothing changes. `gpioReadPin()` reading IDR, `uartFlush()` reading SR, and `pwrWaitVoltageScaleReady()` reading CSR all qualify. `const` here is a claim about the *hardware*, not just about the pointer: it tells a caller the call is repeatable and reorderable with respect to the peripheral.

So a read with a side effect does not get `const`, however read-shaped it looks. `uartReceiveByte()` is the case to know: it reads DR, and reading DR clears SR.RXNE along with the ORE/NE/FE/PE error flags. It took `const UartRegisters_t *` for one revision on the grounds that it never writes through the pointer, which is true and beside the point — the peripheral moves underneath it. It is now non-`const`, and the reasoning is recorded in its doc comment so it does not get "cleaned up" back.

## Testing and its limits

Host tests run driver logic against a plain in-memory register struct. That works because of the rule in the error-handling contract above — every driver takes its register block as a parameter — and because of a second property worth naming: for every bounded hardware wait in this project, the flag being polled is a *different field* from the one the driver writes. `rccHsiEnable()` writes CR.HSION and polls CR.HSIRDY; `pwrWaitVoltageScaleReady()` polls CSR.VOSRDY against a scale written to CR. A test can hold the ready flag clear in the fake struct and exercise the timeout path with no hardware and no timer.

Code that breaks that property is not host-testable, and there is currently exactly one instance: `flashSetLatency()`'s read-back check compares ACR against the value it just wrote to ACR, and an in-memory struct always agrees with itself. That region is marked `GCOVR_EXCL_START`/`GCOVR_EXCL_STOP` so `make coverage` can still gate at 100%, with the reasoning in the code. Before adding a second exclusion, be sure the branch is genuinely unreachable off-target rather than merely untested — the gate is only worth having if 100% means the tests actually exercise the code.

**What host tests cannot tell you**: that the hardware accepts what the driver writes. Every driver in `drivers/` currently holds 100% line and branch coverage and *none of it has executed on silicon* — `app/src/main.c` is an empty loop, so the linker garbage-collects every driver out of the image. That gap is not theoretical. The PWR voltage-scale sequencing shipped inverted for a full release: `rccSysclkSwitch()` applied the scale after requiring PLLRDY, but RM0390 section 5.1.4 permits CR.VOS to be written only while the PLL is *off*, and CSR.VOSRDY only reports the regulator settled once the PLL is *on*. Both halves were wrong, both were covered by passing tests, and a struct in host memory has no opinion about either rule. Treat the reference manual, not the test suite, as the authority on ordering and writability constraints.

## CI triggers and branch conventions

`ci.yml` runs on every `pull_request` (any target branch) and every `push` to `main`/`develop` specifically, not every branch: `make all`, `make format-check`, `make test`, `make coverage`, `make lint`, `make docs`, in that order. `make coverage` is CI-only, not part of the pre-commit hook's five checks - it recompiles `TEST_DRIVER_SOURCES` with instrumentation, a real (if fast) extra compile pass, so it stays out of the hook's every-commit path and lives only in CI. On `pull_request` events specifically, the two diff-based checks above also run, since only a pull request has a base branch to diff against.

`push` is scoped to `main`/`develop` rather than every branch so that a feature branch's CI coverage comes entirely from `pull_request` once a PR is open, never both: `main`/`develop` have required-status-check branch protection (`build-and-check` and `Analyze (c-cpp)`, per [SECURITY.md](SECURITY.md)), and a `push`+`pull_request` double-trigger on the same commit used to be deduped with a `cancel-in-progress` concurrency group - that was removed because a cancelled run's conclusion sticks to the commit alongside a later successful one, leaving the required-check gate permanently `BLOCKED` even though the same-named check also passed. Scoping `push` removes the double-trigger for the case that matters; the residual case (pushing directly to `develop` while an open `develop`→`main` PR exists) is rare, and without `cancel-in-progress` both runs simply complete normally instead of one landing as cancelled.

There is deliberately no hardware-in-the-loop workflow. An earlier `hil.yml` ran `make all` → `make flash` → a smoke test on a self-hosted runner with a Nucleo-F446RE attached, gated behind `workflow_dispatch` so that fork or push code could never execute on that physical machine automatically. It was removed rather than finished: its smoke-test step was never written (it ended in `exit 1`, so the workflow failed by construction on every run), and the board is attached to the development machine anyway, which makes the self-hosted runner a trust boundary the project was carrying for no benefit it was actually collecting.

On-target verification is therefore manual for now. `make flash` programs the board and verifies the write landed; `make debug` attaches GDB for inspecting runtime state by hand. Neither asserts anything about runtime behaviour, and nothing currently does — see "Testing and its limits" above for what that costs. Revisit an automated on-target check once there is behaviour worth asserting against: today `app/src/main.c` is an empty loop and every driver is garbage-collected out of the image for want of a caller, so such a check could only confirm boot-time core registers. Once `api/` can drive the LED or emit a UART heartbeat, a real smoke test has something to observe.
...
`codeql.yml` runs GitHub CodeQL static analysis on every push/PR to `main`/`develop` and weekly on a schedule. This project's actual supply-chain surface is the third-party Actions the workflows use, all pinned to commit SHAs rather than mutable tags, not a C package manager, since none is used. These pins are bumped manually. See [SECURITY.md](SECURITY.md) for the complete trust-boundary reasoning behind all three workflows and the pinning practice.

**Branch convention:** feature/topic branches are merged via pull request into `develop`. `develop` is merged into `main` at release points, not on every merge. See [docs/VERSIONING.md](docs/VERSIONING.md) for release criteria and version numbering.
