...
On-target verification is therefore manual for now. `make flash` programs the board and verifies the write landed; `make debug` attaches GDB for inspecting runtime state by hand. Neither asserts anything about runtime behaviour, and nothing currently does. Revisit an automated on-target check once there is behaviour worth asserting against: today `app/src/main.c` is an empty loop and every driver is garbage-collected out of the image for want of a caller, so such a check could only confirm boot-time core registers. Once `api/` can drive the LED or emit a UART heartbeat, a real smoke test has something to observe.
