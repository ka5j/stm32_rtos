...
**Topics grouping**: the generated docs site (Doxygen's "Topics" page) is organized by architectural layer, not by raw file listing — [docs/groups.dox](docs/groups.dox) declares the group hierarchy and mirrors [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)'s layer diagram exactly, including the groups for layers that don't exist yet (declared there as a naming reference, not yet created as live `@defgroup`s). When a new layer gets its first real file (e.g. `drivers/inc/gpio.h`), add that layer's `@defgroup` to `groups.dox` using the name already reserved for it there, then wrap the new file's struct/macro/function definitions in `@addtogroup <group_id>` / `@{` ... `@}` so they join the group — don't repeat `@ingroup` on every individual member. See `core/inc/`/`device/inc/` for the pattern already in place.
